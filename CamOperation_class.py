...
import os
import platform
from utils import frame_to_numpy, apply, draw_boxes
...
class CameraOperation():
    ...
    def __init__(self, obj_cam, st_device_list, label, widgetDisplay, n_connect_num=0, b_open_device=False, b_start_grabbing=False,
        ...
        self.label = label
        self.widgetDisplay = widgetDisplay
    ...
    def Open_device(self):
        if not self.b_open_device:
            ...
            ret = self.obj_cam.MV_CC_SetIntValue("Width", 2784)
            if ret != 0:
                print("set image width fail! ret[0x%x]" % ret)
            ret = self.obj_cam.MV_CC_SetIntValue("Height", 1856)
            if ret != 0:
                print("set image height fail! ret[0x%x]" % ret)
            ret = self.obj_cam.MV_CC_SetIntValue("OffsetX", 1256)
            if ret != 0:
                print("set offset x fail! ret[0x%x]" % ret)
            ret = self.obj_cam.MV_CC_SetIntValue("OffsetY", 686)
            if ret != 0:
                print("set offset y fail! ret[0x%x]" % ret)
            
            ret = self.obj_cam.MV_CC_SetEnumValueByString("ExposureAuto", "Off")
            if ret != 0:
                print("set auto exposure fail! ret[0x%x]" % ret)
            ret = self.obj_cam.MV_CC_SetFloatValue("ExposureTime", 5000.0)
            if ret != 0:
                print("set exposure time fail! ret[0x%x]" % ret)
            ret = self.obj_cam.MV_CC_SetFloatValue("Gain", 23.9812)
            if ret != 0:
                print("set gain fail! ret[0x%x]" % ret)
            # ret = self.obj_cam.MV_CC_SetEnumValue("PixelFormat", 35127316)
            # if ret != 0:
    ...
    def Work_thread(self, winHandle):
        ...
        while True:
            ...
            if 0 == ret:
                ...
                image = frame_to_numpy(stOutFrame)
            ...
            # Apply detection and counting
            boxes, scores, class_id = apply(image, conf_thres=0.5, iou_thres=0.3)
            ...
            # 5. update widgetDisplay (thread-safe)
            QMetaObject.invokeMethod( self.widgetDisplay, "setPixmap", Qt.QueuedConnection, 
                                     Q_ARG(QPixmap, QPixmap.fromImage(qimg)))

            QMetaObject.invokeMethod(self.label, "setText", Qt.QueuedConnection, Q_ARG(str, str(len(boxes))))
            # 是否退出
            if self.b_exit:
                if self.buf_save_image is not None:
                    del self.buf_save_image
